...
> **Canonical copy:** [`docs/phase-wise-architecture.md`](docs/phase-wise-architecture.md) (includes Phase 4 implementation status and Phase 5 plan).
...
### API surface (illustrative REST)

| Method | Path | Body | Response |
|--------|------|------|----------|
| `POST` | `/v1/recommend` | JSON PreferenceDTO | RecommendationResult + HTTP 200 |
| `GET` | `/health` | — | `{ "status": "ok", "dataset": "..." }` |

Error model: `400` validation, `503` LLM unavailable (optional retry hint), `500` unexpected.
...
- Runnable server or CLI entrypoint.
- OpenAPI spec or README examples (`curl`).

### Verification

- Integration test: POST with fixture data → 200 and schema match.
- Health check returns after dataset load.
...
- Implemented UI or CLI consuming the service contract.
- Style aligned with a minimal design system or consistent CSS tokens.

### Verification

- Manual or E2E test: submit preferences → see five fields populated per recommendation.
...
*Document version: 1.1 — aligned with `docs/problemstatement.md`.*
